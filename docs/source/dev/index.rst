...
The ipy-course-tools project implements a regression
...
not broken existing functionality. The ipy-course-tools
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make test

To run tests verbosely use:

.. code-block:: console

    (ipy_course_tools) $ make test-verbose
...
``ipy_course_tools`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``ipy_course_tools`` package can be found.

.. code-block:: console

    (ipy_course_tools) $ cd tests
    (ipy_course_tools) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (ipy_course_tools) $ python -m test_version
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make coverage
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make sort-imports
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make check-static-analysis
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make check-lint
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make check-types
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make check-docs
...
.. code-block:: console
    ...
    (ipy_course_tools) $ make serve-docs
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/ipy_course_tools/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (ipy_course_tools) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (ipy_course_tools) $ make dist-upload

  The package should now be available at https://pypi.org/project/ipy_course_tools/
