...
# help:
# help: ipy-course-tools Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt ipy_course_tools
...
.PHONY: format
format:
	@black src/ipy_course_tools tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/ipy_course_tools tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p ipy_course_tools --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc ipy_course_tools ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/ipy_course_tools*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./ipy_course_tools-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/ipy_course_tools-*-py3-none-any.whl
